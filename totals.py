...
class FinalCalculations(DiseaseProgression):
    ...
    def get_total_life_years(self, cohort):
        ...
        """
        return self.disease_progression.loc[cohort].to_numpy().sum()

    def get_total_QALY(self, cohort):
        """ Calculate total QALY by summing each column of progression values, weighing these values and summing them. """
        return self.disease_progression.loc[cohort].sum(axis=0).multiply(
            self.scoring_and_costing['Quality Weight']).sum()

    def get_total_cost(self, cohort):
        """ Calculate total cost by summing each column of progression values, weighing these values with a cost, summing them and
            then adding the total difference of death progression (delta death). """

        first_year_costs = self.scoring_and_costing.loc['death']['First year costs']

        total_cost = self.disease_progression.loc[cohort].sum(axis=0).multiply(
            self.scoring_and_costing['PPPY cost']).sum()
    ...
    def get_totals(self):
        ...
        for cohort in self.total_age_cohorts:
...
#     icer_calculation = FinalCalculations().write_totals_to_csv()
